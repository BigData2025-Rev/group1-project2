...
def create_location_sales_trend(row):
    ...
    new_row = Row(order_id=row.order_id, customer_id=row.customer_id, customer_name=row.customer_name, product_id=row.product_id, product_name=row.product_name, product_category=row.product_category, payment_type=row.payment_type, qty=row.qty, price=row.price, datetime=row.datetime, country=COUNTRY, city=CITY, ecommerce_website_name=row.ecommerce_website_name, payment_txn_id=row.payment_txn_id, payment_txn_success=row.payment_txn_success, failure_reason=row.failure_reason)
    return new_row
